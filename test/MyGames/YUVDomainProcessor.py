...
class FalseColorSuppression:
    ...
    def execute(self):
        ...
        self.img[(em > self.fcs_edge[0]) & (em <= self.fcs_edge[1])] *= 1 - (
                self.img[(em > self.fcs_edge[0]) & (em <= self.fcs_edge[1])] - self.fcs_edge[0]) \
...
class HueSaturationControl:
    ...
    def execute(self):
        ...
        self.img = np.einsum('ijk,kl->ijl', self.img,
                             np.array([[np.cos(self.hue), -np.sin(self.hue)], [np.sin(self.hue), np.cos(self.hue)]]))
